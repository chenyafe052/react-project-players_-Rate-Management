...
export default class PlayerByRateTeam extends React.Component {
  ...
  send = event => {
    ...
    let params = {
      team: this.team.team_name.value,
      rate: this.rate.value
    };

    const url = `https://playersratemanagement1.herokuapp.com/getPlayerByRateTeam`;
    fetch(url, {
      method: "PuT",
      headers: {
      ...
      <div>
        <form className="ui form">
          <div className="field">
            ...
            <select ref={input => (this.team.team_name= input)}>
              {" "}
          ...
          <div className="field">
            <label>rate</label>
            <input type="text" ref={input => (this.rate = input)} />
          </div>
